...
import { FederalRegisterDataDO } from "./do";

export { FederalRegisterDataDO };

interface FederalRegisterEnv {
...
export class MyMCP extends StatelessMcpWorker {
	...
	async init() {
		...
		const env = this.env as unknown as FederalRegisterEnv;
...
export default {
	fetch(request: Request, env: Env, ctx: ExecutionContext) {
		...
		if (url.pathname === "/health") {
			return buildHealthResponse("federal-register");
		}
		...
		if (url.pathname === "/readyz") {
			...
			return MyMCP.readiness(env, "federal-register");
		}
